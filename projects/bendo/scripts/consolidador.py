...
def consolidar(carpeta: Path) -> pl.DataFrame:
    ...
    consolidado_facturas = consolidado_facturas.with_columns(pl.col("numero_ruc").cast(pl.Int64))
    contactabilidad = contactabilidad.with_columns(pl.col("numero_ruc").cast(pl.Int64))

    # Joins sucesivos
    df = (
        df_info_general
        .join(consolidado_facturas, on=["numero_ruc", "numero_establecimiento"], how="left")
